...
pushd tpch-scripts/03_run

##### RUN W/ PROFILING

# start the db w/ profiling enabled
./start_mserver.sh --farm /home/centos/tpch-scripts/monetdb-farm --db SF-$scaling_factor --set gdk_nr_threads=$num_threads --stethoscope --verbose
...
# start the db w/ profiling enabled
./start_mserver.sh --farm /home/centos/tpch-scripts/monetdb-farm --db SF-$scaling_factor --set gdk_nr_threads=$num_threads --verbose
